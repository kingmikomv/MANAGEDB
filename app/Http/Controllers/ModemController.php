...
class ModemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Cari modem berdasarkan serial_number
        $existing = Modem::where('serial_number', $request->serial_number)->first();

        if ($existing) {
            // Jika modem sudah terpasang
            if ($existing->status === 'terpasang') {
                return response()->json([
                    'exists' => true,
                    'installed' => true,
                    'message' => 'Modem sudah terpasang pada pelanggan lain!',
                ]);
            }

            // Jika modem hanya sudah terdaftar (tersedia)
            return response()->json([
                'exists' => true,
                'installed' => false,
                'message' => 'Nomor seri sudah ada di database.',
            ]);
        }

        // Jika modem belum ada → buat baru
        $modem = Modem::create([
            'serial_number' => $request->serial_number,
            'status' => $request->filled('pelanggan') ? 'terpasang' : 'tersedia',
            'pelanggan_aktif' => $request->pelanggan,
        ]);

        // Jika pelanggan diisi → tambahkan ke riwayat
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data modem berhasil disimpan!',
        ]);
    }
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Modem berhasil dihapus!',
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menghapus modem: '.$e->getMessage(),
            ]);
        }
    }
}
